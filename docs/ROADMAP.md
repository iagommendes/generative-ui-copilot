...
| Entrega | Detalhe |
| --- | --- |
| `generativeRegistry` | Mapa `toolName → { schema, component, mcpKind }` |
| Geração de tools | `buildTools(registry)` usado pelo `streamUI` |
| Novos widgets | `DataTable`, `KpiStat`, `LineChart` (mínimo 1 novo) |
| Docs de contribuição | Como registrar um componente generativo em <10 linhas |

**Critério de pronto:** um contributor adiciona `DataTable` só editando registry + arquivo do componente.
